...
构建、类型检查、17 项测试及双轴复审已完成。已提交代码在干净目录以 `npm ci --ignore-scripts` 安装、构建、类型检查并通过查询隔离测试；仍需默认安装全流程、扩展乱序/空结果/超限覆盖、完整无密钥录制回放、真实模型输入预算与长会话验证、回答质量改进及全流程 UI 验收。字节上限属于当前保守预检，不代表已经验证模型的完整上下文预算。
