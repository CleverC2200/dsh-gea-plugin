...
使用启动日志中的带认证参数链接进入页面，左侧直接显示 GEA 业务导航。中间扫码登录后加载原销售计划审批工作台；选择组织行与分析范围，点击「预览发送范围」，核对 Host 重新获取的当前计划数据，再确认送入右侧原生 DSH 会话。选中 Session 不会关闭业务页面，后续输入、流式回答、取消和历史记录仍由 DSH 负责。
...
此前的真实 GEA/模型验证记录见 [早期验收记录](docs/acceptance-2026-09-10.md)，它不能替代本次 fork + 原工作台的真实环境重新验收。当前仍需真实登录后核对审批队列、版本/组织范围及原生模型会话；独立安装包、写操作和生产验收也仍未完成。
