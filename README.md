...
消息待办支持独立的只读列表、未读数、分页与详情，读取 GEA `/api/v1/notifications`，保留来源引用与原始通知状态。打开详情不会标记已读或执行审批；来源引用不自动解释成 DSH Session。身份切换取消在途查询，响应失败清除旧页面结果。当前验收使用模拟 GEA 与真实 DSH/browser，实际部署接口和来源会话映射仍待验证。
