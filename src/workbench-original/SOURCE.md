...
Workflow projection and approval-window/SAVE presentation updates follow AionUi commit `3e5a782b4`. The plugin owns its transport: the authenticated DSH Host creates a GEA delegation session and reads only `agents_scm_plan_workflow_config` through a fixed SELECT. No Electron transport or AionUi login dependency is shipped. Live configuration preserves non-contiguous levels, maps standard node roles, and replaces the document projection before periods load; failure does not restore the static snapshot. DSH business writes remain disabled.
