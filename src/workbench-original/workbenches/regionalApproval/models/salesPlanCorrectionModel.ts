...
import { salesPlanWorkflow, type SalesPlanRole } from '../../../salesPlanWorkflow.ts';
...
export type CorrectionFields = Partial<
  Record<`${CorrectionNode}AdjAddQty` | `${CorrectionNode}AdjCutQty` | 'adjAddQty' | 'adjCutQty', string | null>
>;
export type CorrectionSku = Pick<GeaSalesPlanSku, 'qty' | 'price'> & CorrectionFields;
export function correctionDecimal(value: unknown): Decimal {
  if (typeof value !== 'string' || !/^[+-]?\d+(?:\.\d+)?$/.test(value)) throw new Error('纠偏数据缺失或格式错误');
...
export function correctionDecision(
  ...
  const roles = salesPlanWorkflow(typeCode).confirmationRoles(status);
  for (const role of includeCurrent ? roles : roles.slice(1)) {
    ...
    if (a.isNegative() || c.isNegative() || (!a.isZero() && !c.isZero())) throw new Error('纠偏调整必须是绝对净量');
    return a.minus(c).toFixed();
  }
  throw new Error('纠偏继承数据缺失');
}
export function correctionLine(sku: Pick<GeaSalesPlanSku, 'qty' | 'price'>, input: string) {
  if (!/^[+-]?\d{1,15}(?:\.\d{1,3})?$/.test(input)) throw new Error('调整量最多三位小数');
  ...
  if (base.isNegative() || price.isNegative() || qty.isNegative()) throw new Error('纠偏后计划数量不可为负');
...
export function correctionMetrics(row: {
  ...
  const ratio = (n?: string, d?: string) => {
    try {
      ...
      return denominator.gt(0) ? correctionDecimal(n).div(denominator).times(100).toFixed(2) : undefined;
    } catch {
      return undefined;
    }
  };
  ...
  try {
    netAmount = correctionDecimal(row.corrPlanAmount).minus(correctionDecimal(row.mPlanAmount)).toFixed(2);
  } catch {
    /* Missing is not zero. */
  }
  return {
...
import type { GeaSalesPlanDetail, GeaSalesPlanActionRequest, GeaSalesPlanActionReceipt } from '../../../contracts.ts';

export function correctionAccess(detail: GeaSalesPlanDetail) {
  const { currentVersion: v, actionContext: c, correctionContext: policy } = detail;
  const role = salesPlanWorkflow(v.planTypeCode).actor(v.status);
  return v.orderType === 'Z' &&
    ...
    c?.versionId === v.id &&
    c.status === v.status &&
    c.nodeOrder === ['customer', 'region', 'province', 'area', 'category'].indexOf(role) + 1 &&
...
export function correctionAdjustments(detail: GeaSalesPlanDetail, edits: Record<string, string>) {
  return detail.skus.flatMap((sku) => {
    const delta =
      edits[sku.skuCode] ?? correctionDecision(sku, detail.currentVersion.status, detail.currentVersion.planTypeCode);
    const line = correctionLine(sku, delta);
    return line.participates ? [{ skuCode: sku.skuCode, adjustQty: line.delta }] : [];
  });
}
...
export function verifyCorrectionAction(
  ...
  if (
    ...
    return false;
  const decisions = new Map((request.adjustments ?? []).map((x) => [x.skuCode, x.adjustQty]));
  if (
    new Set(after.skus.map((x) => x.skuCode)).size !== after.skus.length ||
    after.skus.some((x) => x.versionId !== v.id || !before.skus.some((s) => s.skuCode === x.skuCode))
  )
    return false;
  ...
  return before.skus.every((sku) => {
    ...
    if (request.action !== 'REJECT' && delta == null && correctionLine(sku, '0').participates === false) return !saved;
    if (!saved || !equal(saved.qty, sku.qty) || !equal(saved.price, sku.price)) return false;
    for (const other of ['region', 'province', 'area', 'category'] as const) {
      for (const suffix of ['AdjAddQty', 'AdjCutQty', 'ConfirmedQty', 'ConfirmedAmount'] as const) {
        if (other !== role || (request.action === 'SAVE' && suffix.startsWith('Confirmed'))) {
          if (!equal(saved[`${other}${suffix}`], sku[`${other}${suffix}`])) return false;
        }
      }
    }
    if (request.action === 'REJECT')
      return ['AdjAddQty', 'AdjCutQty', 'ConfirmedQty', 'ConfirmedAmount'].every(
        (s) => saved[`${role}${s}` as keyof GeaSalesPlanSku] == null,
      );
    ...
    return (
      ...
      (request.action === 'SAVE' ||
        (equal(saved[`${role}ConfirmedQty`], expected.qty) && equal(saved[`${role}ConfirmedAmount`], expected.amount)))
    );
  });
}
