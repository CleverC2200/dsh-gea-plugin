...
`tests/notifications.test.mjs` 在真实 DSH profile 与 Chrome、模拟外部 GEA 下验证列表/详情、未读数、部分覆盖、字段投影、固定路由、重复记录、身份不符、403、空结果、登录过期和切换环境取消。原审批页由 tests/web.test.mjs 回归。此证据不包含生产通知实测、来源 Session 映射或写入验收。
