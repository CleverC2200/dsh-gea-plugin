...
The NSIS installer uses ZIP extraction directly into the installation directory. The previous 7z template extracted to a temporary tree and then copied every file. Windows package verification also exposed a Unix-only archive containment check; Agent Manage 0.6.3-company.4 uses the platform separator and keeps traversal rejection.
