...
test('correction uses a fixed monthly baseline and absolute node decisions including explicit zero', () => {
  ...
  assert.equal(correctionDecision({ ...sku, provinceAdjAddQty: '0', provinceAdjCutQty: '0' }, 2, 'Y'), '0');
  ...
  assert.equal(correctionLine({ qty: '0', price: '1' }, '0').participates, false);
...
test('actual progress keeps the monthly denominator and corrected progress measures target coverage', () => {
  assert.deepEqual(
    ...
    { amountProgress: '50.00', qtyProgress: '25.00', correctionProgress: '90.00', netAmount: '100.00' },
  );
  assert.equal(correctionMetrics({ targetAmount: '0', mPlanAmount: '0', mPlanQty: '0' }).correctionProgress, undefined);
});
