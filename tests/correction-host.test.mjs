...
test(
  ...
  async (t) => {
      ...
      if (req.url.pathname.endsWith('/plans/z')) {
        reply({
          ...
          result: {
            ...
            correctionContext: { contract: 'absolute-net-v1', monthlyApproved: eligible, approvalOpen: true },
            skus: [{ id: '1', versionId: 'v', skuCode: '123', qty: '30', price: '2', adjAddQty: '2', adjCutQty: '0' }],
            versions: [],
            logs: [],
          },
        });
    ...
    eligible = false;
    assert.equal((await app.rpc('sales-plan/action', { ...payload, idempotencyKey: 'denied' })).ok, false);
    assert.equal(writes, 1);
  },
);
